  ...
  **BOJ 4991 - 로봇 청소기**
  ...
  - **문제 힌트 (Problem Hint):**
    ...
    **BOJ 6118 - 숨바꼭질**
    ...
    - **Key Idea:**
     - `dist[1]=0`, 나머지 `-1`로 두고 BFS 한 번으로 거리 채운 뒤,
     - `dist`를 스캔해 `mx`, 그 거리의 **가장 작은 정점** `idx`, **개수** `cnt`를 구함.

    - **Caution:**
     - 비연결 그래프여도 미방문은 `-1`이라 결과엔 영향 없음.
     - 한 패스로 구할 땐 `if (dist[i] > mx) { mx=..., idx=i, cnt=1} else if (dist[i]==mx) ++cnt;`.

    - **Improvement:**
     - 두 번 스캔 대신 한 번에 `mx/idx/cnt`를 계산해도 됨(가독성 취향).

    - **Problem Hint:**
     - 무가중치 -> BFS가 최단, 추가 정렬 불필요.
    ...
    **BOJ 16928 - 뱀과 사다리 게임**
    ...
    - **Folder:** `graph/bfs/`
    - **Time Complexity:** O(V + E) ≈ O(100 + 600) (상수)
    - **Space Complexity:** O(V)

    - **Key Idea:**
     - 칸을 상태로 보고, 주사위 1..6 이동 후 **즉시 점프를 적용**한 목적지 상태에 대해 BFS 방문/거리 갱신.

    - **Caution:**
     - **점프 적용 -> 방문 판정 순서**를 반드시 지킬 것.
       잘못된 순서(방문 판정 후 점프)는 실제 도달 상태를 마킹하지 못해, 같은 목적지가 여러 경로로 **반복 삽입**됨.
       뱀/사다리가 이루는 **사이클**이 존재할 경우 큐가 계속 불어나며 **무한 루프처럼 보이는 현상**이 남.
     - 방문 조건은 `if (count[next] != -1) continue;`로 단순화해도 정답 동일.

    - **Improvement:**
     - `jump` 미정의 칸은 -1, 정의된 칸만 목적지로 치환하는 현재 방식 유지.
     - 시작 칸을 1로 두고 구현해도 무방하나, 현재 0 + 주사위 진입도 논리적으로 동등.

    - **Problem Hint:**
     - 상태 공간이 작아(100칸) BFS 한 번이면 충분하며, **"이동 후 점프"를 적용한 최종 상태**만을 방문 처리하면 된다.
    ...
    **BOJ 16236 - 아기 상어**
    ...
    - **Key Idea:** 
     - 라운드마다 BFS로 거리 맵을 만들고, 먹잇감 후보를 **최단거리 -> 행 -> 열** 우선으로 선택.
     - 시간은 `cur_time += t[target]`로 누적.

    - **Caution:**
     - 시작 칸 9 -> 0 처리 필수.
     - `dist`가 정해지면 그보다 **깊은 레벨 확장 금지**.
     - 사이즈 업은 "먹은 수 == 현재 크기"일 때만.

    - **Improvement:**
     - 미세 최적화: 큐 팝 시 `if (dist != -1 && t[cx][cy] >= dist) continue;`로 더 일찍 자르기 가능.
     - 후보를 정렬 대신 **반복 중 최소 (행, 열) 갱신**으로 뽑으면 약간 더 빠름.

    - **Problem Hint:**
     - "가장 가까운" + "위-왼쪽" 타이브레이크만 정확히 구현하면 된다.
    ...
    **BOJ 13460 - 구슬 탈출 2**
    ...
    - **Key Idea:**
     - `visited[rx][ry][bx][by]`로 상태 중복을 제거하면서 BFS로 최단 "기울이기 횟수" 탐색.
     - 한 방향으로 굴린 결과에서
      - 파랑이 구멍(O) -> 버림
      - 빨강만 구멍(O) -> 정답
      - 겹치면 이동거리 비교로 한 칸 뒤로 조정

    - **Caution:**
     - 굴리기 while에서 경계 체크를 안 하므로, 보더가 `#`라는 전제(지문 보장)가 깨지면 **배열 범위 밖 접근(out of bounds)** 위험.
     - 시작 상태 `visited`를 안 찍으면 중복 탐색이 생길 수 있음(정답은 맞더라도 불필요).
     - `cnt == 10`일 때는 더 확장하면 11이 되므로 확장 금지.

    - **Improvement:**
     - 시작 상태도 `visited[start] = true;`
     - `if (ccnt == 10) break;` 대신 `continue;`가 의미상 더 정확(큐에 남은 다른 10-depth 상태를 "처리"할 필요는 없지만, 습관상 안전한 형태).
     - 입력에서 `R`, `B`를 찾은 뒤 보드에서 `'.'`로 바꿔두면 가독성이 더 좋아짐(로직은 동일).
     - 굴리기 로직을 함수로 빼면 실수 줄고 코드가 짧아짐.

    - **Problem Hint:**
     - "기울이기 1회" = 간선 비용 1 -> BFS
     - 상태는 (R 위치, B 위치) 4차원 방문으로 관리
    ...
    **BOJ 13549 - 숨바꼭질 3**
    ...
    - **Key Idea:**
     - 0초 간선(2x)과 1초 간선(+1, -1)이 섞인 최단거리 문제.
     - 다익스트라로 `dist[next]`를 갱신하며 최소 시간을 구함.

    - **Caution:**
     - 범위(0~100000) 체크 필수 -> 배열 범위 밖 접근 (out of bounds) 방지.
     - 우선순위 큐에 "오래된 값(stale)"이 들어갈 수 있으므로 `if (d != dist[x]) continue;`  권장.

    - **Improvement:**
     - 간선 가중치가 0/1이므로 **0-1 BFS(deque)**로 `O(V + E)`에 더 빠르게 해결 가능.
     - `n >= k`면 답은 `n-k` (순간이동은 증가만 하므로 이득 없음)으로 빠른 종료 가능.

    - **Problem Hint:**
     - 가중치가 0 또는 1-> 0-1 BFS 또는 다익스트라.
    ...
    **BOJ 13913 - 숨바꼭질 4**
    ...
    - **Key Idea:**
     - BFS로 `dist[next] = dist[cur] + 1`, `parent[next] = cur`를 기록.
     - `k`에서 시작해 `parent`를 따라가며 경로를 역추적 후 뒤집어 출력.
    ...
    - **Improvement:**
     - `dist`/`parent`를 분리 배열로 두면 의미가 더 명확.
     - 경로 복원 시 입력 변수 `k`를 덮어쓰지 말고 `cur`같은 변수 사용.

    - **Problem Hint:**
     - "최단 시간 -> BFS
     - "경로 출력" -> parent 저장 후 역추적
    ...
    **BOJ 9019 - DSLR**
    ...
    - **Topic:** BFS | Path Reconstruction
    - **Folder:** `graph/bfs/`
    - **Time Complexity:** O(10000) per test
    - **Space Complexity:** O(10000)

    - **Key Idea:**
     - BFS로 최단 경로를 찾고, `visited[next] = {op, parent}`로 복원.

    - **Caution:**
     - S 연산 구현 실수 주의: `cur==0 ? 9999 : cur-1`
     - 상태 공간이 10000으로 작아 BFS가 안정적.

    - **Improvement:**
     - D는 `(cur*2)%10000`으로 단순화.
     - 시작점 방문 표시를 해두면 중복 탐색 감소
